...
module Skirmish::Automation
  ...
  class MatchError < ScriptError; end
  ...
  module CliRunner
    ...
    #
    # Throws +MatchError+, +Timeout::Error+ Errno::EPIPE or +EOFError+
    # on failure
    #
    def >> pattern, timeout = cli_option(:default_timeout)
      ...
      begin Timeout::timeout(timeout) do
          ...
          raise MatchError unless match = (resp.match(pattern))
        end
      rescue MatchError, Timeout::Error, Errno::EPIPE, EOFError => e
